...
def stampapartita(tipomov, tipodocumento)
	#puts tipomov
	#puts @documento
	#puts orientamento
#	foglio = PDF::Writer.new(:paper => "A4", :orientation => :"{orientamento}") # , :font_size => 5)
	querydoc = ""
	if tipomov == "I" and tipodocumento == "mod4"
		querydoc = "and mod4ingr = ?"
	elsif tipomov == "I" and tipodocumento == "certsan"
		querydoc = "and certsaningr = ?"
	elsif tipomov == "I" and tipodocumento == "rifloc"
		querydoc = "and provenienza = ?"
	elsif tipomov == "U" and tipodocumento == "mod4"
		querydoc = "and mod4usc = ?"
	elsif tipomov == "U" and tipodocumento == "certsanusc"
		querydoc = "and certsanusc = ?"
	end


	#puts "qua"
	#puts querydoc
	capi = Registros.find(:all, :conditions => ["relaz_id = ? #{querydoc}", "#{@stallaoper.id}", "#{@documento}"])
...
#	foglio.start_page_numbering(10, 10, 6, :center, "<PAGENUM>/08", 1)
	foglio.open_object do |testa|
		...
		testo = "STALLA #{@stallaoper.stalle.cod317} - DOCUMENTO #{@documento} - NUMERO CAPI: #{@totcapi}</b>"
...
#	tabella.title = "PDF dati"
	...
	capi.each do |i, index|
		if tipodocumento == "U"
			...
			data << {"prog" => i["progressivo"], "marca" => i["marca"], "razza" => i["razza"], "sesso" => i["sesso"], "madre" => i["madre"],	"nc" => i["tipoingresso"], "nascita" => i["datanascita"], "ingresso" => i["dataingresso"], "prov" => i["provenienza"]}
		else
			...
			if i["datauscita"].to_s != ""
				uscitacapo = i["datauscita"].strftime("%d/%m/%Y")
			else
				uscitacapo = ""
			end
#			data << {"prog" => "#{i.prog}", "marca" => "#{i.marca}", "razza" => "#{i.razza}", "sesso" => "#{i.sesso}", "madre" => "#{i.madre}", "nc" => "#{i.tipoingresso}", "nascita" => "#{i.datanascita}", "ingresso" => "#{i.dataingresso}", "prov" => "#{i.provenienza}", "motusc" => "#{i.tipouscita}", "datausc" => "#{uscitacapo}", "destinazione" => "#{destinazione}", "marcaprec" => "#{i.marcaprec}", "mod4" => "#{mod4pul}"}
			data << {"prog" => i["progressivo"], "marca" => i["marca"], "razza" => i["razza"], "sesso" => i["sesso"], "madre" => i["madre"], "nc" => i["tipoingresso"], "nascita" => i["datanascita"].strftime("%d/%m/%Y"), "ingresso" => i["dataingresso"].strftime("%d/%m/%Y"), "prov" => i["provenienza"], "motusc" => i["tipouscita"], "datausc" => "#{uscitacapo}", "destinazione" => "#{destinazione}", "marcaprec" => i["marcaprec"], "mod4" => "#{mod4pul}"}
		end
	end
	#foglio.margins_mm(15, 10, 10)
	#puts data.length
	if data.length != 0
		...
		foglio.save_as("#{@dir}/altro/partita.pdf")
		if @sistema == "linux"
			system("evince #{@dir}/altro/partita.pdf")
		else
#			foglio.save_as('.\registro\registro_ingresso.pdf')
			@shell.ShellExecute('.\altro\partita.pdf', '', '', 'open', 3)
		end
	end
=begin
