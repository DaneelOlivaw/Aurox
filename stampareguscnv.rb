def registrouscnv(comboanno)
	...
	fogliousc.open_object do |testa|
		...
		testo = "<b>Registro non vidimato di scarico della stalla #{@stallaoper.stalle.cod317} di #{@stallaoper.ragsoc.ragsoc}</b>"
	...
	selcapi = Registros.find(:all, :conditions => ["contatori_id= ? and tipouscita != 'null' and YEAR(datauscita) = ?", "#{@stallaoper.contatori_id}", "#{comboanno.active_iter[0]}"], :order => ["datauscita, mod4usc, id"])
