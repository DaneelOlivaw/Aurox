...
def stampareggenerico(selreg, datainizio, datafine, orientamento)
	#puts orientamento
...
#	foglio.start_page_numbering(10, 10, 6, :center, "<PAGENUM>/08", 1)
	foglio.open_object do |testa|
		...
		testo = "<b>Movimenti della stalla #{@stallaoper.stalle.cod317} di #{@stallaoper.ragsoc.ragsoc} dal #{datainizio} al #{fine}: #{selreg.length}</b>"
...
#	tabella.title = "PDF dati"
	...
	selreg.each do |i, index|
		if orientamento == "portait"
			...
			data << {"prog" => "#{i.progressivo.to_s}", "marca" => "#{i.marca}", "razza" => "#{i.razza}", "sesso" => "#{i.sesso}", "madre" => "#{i.madre}", "nc" => "#{i.tipoingresso}", "nascita" => "#{i.datanascita.strftime("%d/%m/%Y")}", "ingresso" => "#{i.dataingresso.strftime("%d/%m/%Y")}", "prov" => "#{i.provenienza}", "motusc" => "#{i.tipouscita}", "datausc" => "#{uscitacapo}", "destinazione" => "#{destinazione}", "marcaprec" => "#{i.marcaprec}", "mod4" => "#{mod4pul}"}
		end
	end
	#foglio.margins_mm(15, 10, 10)
	if data.length != 0
