...
def uscmacellazione(finestra, muscite, listasel, combousc, modo)
	...
	listamacdest = Gtk::ListStore.new(Integer, String, String, String, String, String, String)
	...
	selmacdest.each do |macdest|
		...
		itermacdest[4] = macdest.via
		itermacdest[5] = macdest.comune
		itermacdest[6] = macdest.provincia
	end
...
#		end
	...
	unless @mod4provv == ""
		...
		unless @mod4provv[6].to_i == 0
			combomacdest.set_active(0)
			combotrasp.set_active(0)
			z = -1
			while combomacdest.active_iter[0] != @mod4provv[6]
				z+=1
				combomacdest.set_active(z)
			end
		end
		z = -1
	...
	if errore == nil
		...
		if modo == 0
			...
			@mod4provv == ""
			#Mod4temps.execute("ALTER TABLE `mod4temps` AUTO_INCREMENT =1")
			#puts @mod4provv[10]
			Conferma.conferma(mdatimacellazione, "Capi usciti correttamente.")
		else
			if @mod4provv == ""
				Mod4temps.create(:relaz_id => "#{@stallaoper.id.to_i}", :mod4 => "#{@stallaoper.stalle.cod317}/#{Time.parse("#{datamod4uscingl}").strftime("%Y")}/#{mod4usc.text}", :capi => "#{stringacapi.chop}", :datamod4 => "#{datamod4uscingl.to_i}", :datausc => "#{datauscingl}", :macelli_id => "#{valcombomacdest}",:uscite_id => "#{combousc.active_iter[0]}", :naz_dest => "#{valcombonazdest}", :trasportatore => "#{valcombotrasp}")
			else
				Mod4temps.update(@mod4provv[10], {:mod4 => "#{@stallaoper.stalle.cod317}/#{Time.parse("#{datamod4uscingl}").strftime("%Y")}/#{mod4usc.text}", :capi => "#{stringacapi.chop}", :datamod4 => "#{datamod4uscingl.to_i}", :datausc => "#{datauscingl}", :macelli_id => "#{valcombomacdest}",:uscite_id => "#{combousc.active_iter[0]}", :naz_dest => "#{valcombonazdest}", :trasportatore => "#{valcombotrasp}"})
			end
			Conferma.conferma(mdatimacellazione, "Modello 4 provvisorio creato, ora puoi stamparlo.")
			arrmarche = []
			listasel.each do |model,path,iter|
				arrmarche << iter[2]
			end
			require 'stampamod4provv'
			stampamod4provv(mdatimacellazione, arrmarche, combousc.active_iter, combomacdest.active_iter, nil, combotrasp.active_iter, @stallaoper.stalle.cod317 + "/" + Time.parse("#{datamod4uscingl}").strftime("%Y") + "/" + mod4usc.text, Time.parse("#{datauscingl}").strftime("%d/%m/%Y"))
		end
