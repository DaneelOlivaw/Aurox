...
#	foglio.start_page_numbering(10, 10, 6, :center, "<PAGENUM>/08", 1)
	foglio.open_object do |testa|
		...
		testo = "CAPI PRESENTI DA ALMENO #{giorni} GIORNI: #{numcapi}</b>"
...
#	tabella.title = "PDF dati"
	...
	if data.length != 0
		...
		foglio.save_as("#{@dir}/altro/registro_presgiorni.pdf")
		if @sistema == "linux"
			system("evince #{@dir}/altro/registro_presgiorni.pdf")
		else
#			foglio.save_as('.\registro\registro_ingresso.pdf')
			@shell.ShellExecute('.\altro\registro_presgiorni.pdf', '', '', 'open', 3)
		end
	end
=begin
