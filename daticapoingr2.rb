def inscapo2(containgressi, depositoingr)
	...
	finestraingr2.add(boxingrvert)
	if depositoingr["tipomovimento"] != 1
...
#		boxingr10.hide
		...
		@combonaznas.set_active(0)
		contanaz = -1
		#if depositoingr["tiponazprov"] == 1
			#contanaz+=1
		while @combonazorig.active_iter[2] != depositoingr["nazprov"]
			contanaz+=1
			@combonazorig.set_active(contanaz)
			#@combonaznas.set_active(contanaz)
		end
...
#			#puts errore
#			Errore.avviso(finestraingr2, "Controllare la marca")
#		end
#	else
#	end
#	}
	...
	bottmoving = Gtk::Button.new("Inserisci capo")
...
#	end
	...
	if controllo.has_value?("#{razza.text.upcase}")
		...
		Errore.avviso(finestraingr2, "La razza inserita non è classificata.")
		errore = 1
		razzaid = 0
	...
	if marca.text == "" or razzaid == 0 or nascita.text == "" or @combonazorig.active == -1 or @combonaznas.active == -1 or @madre.text == ""
		...
		Errore.avviso(finestraingr2, "La data di nascita non può essere successiva al giorno odierno.")
		errore = 1
		#puts "nato nel futuro"
	elsif Time.parse("#{@datanasingl}") > Time.parse("#{depositoingr["dataingringl"]}")
		Errore.avviso(finestraingr2, "La data di nascita non può essere successiva alla data di ingresso.")
		errore = 1
		...
		if Time.parse("#{@datamarcingl}") < Time.parse("#{@datanasingl}")
			Errore.avviso(finestraingr2, "La data di marcatura non può essere minore della data di nascita.")
			errore = 1
			#puts "marcatura"
		else
		end
...
##			puts "nascita"
#			#@nnaz = "IT"
#			mascnascita(finestraingr2, labelingr)
#		elsif depositoingr["tipomovimento"] == 13 or depositoingr["tipomovimento"] == 32
##			puts "prima importazione"
#			#@nnaz = @combonaznas.active_iter[2]
#			mascprimimp(finestraingr2, labelingr)
#		else
##			puts "altro"
#			@nnaz = "IT"
#			mascingressi(finestraingr2, labelingr)
#		end
	end
	}
...
=begin
	...
	#				end
					...
					if marca.text == "" or @madre.text == "" or razzaid == 0 or nascita.text == "" or @combonazorig.active == -1 or @combonaznas.active == -1
						Errore.avviso(finestraingr2, "Mancano dei dati obbligatori.")
						errore = 1
					elsif marca.text[0,2].upcase == "IT" and marca.text.length < 14
						Errore.avviso(finestraingr2, "Marca corta.")
						...
						Errore.avviso(finestraingr2, "lettere.")
						errore = 1
...
#						Animals.create(:relaz_id => "#{@stallaoper.id.to_i}", :tipo => "I", :cm_ing => "#{@depositoingr["ingresso"]}", :marca => "#{marca.text.upcase}", :specie=> "#{valspecie}", :razza_id => "#{razzaid}", :data_nas => "#{@datanasingl.to_i}", :stalla_nas => "#{stallanas.text.upcase}", :sesso => "#{@valsesso}", :naz_orig => "#{@combonazorig.active_iter[2]}", :naz_nasprimimp => "#{@combonaznas.active_iter[2]}", :data_applm => "#{@datamarcingl.to_i}", :ilg => "#{@valgen}", :embryo => "#{@valembryo}", :marca_prec => "#{@prec.text.upcase}", :marca_madre => "#{@madre.text.upcase}", :marca_padre => "#{@padre.text.upcase}", :donatrice => "#{@don.text.upcase}", :clg => "#{@libgen.text.upcase}", :data_ingr => "#{@depositoingr["dataingr"]}", :naz_prov => "#{@depositoingr["nazprov"]}", :certsan => "#{@depositoingr["certsan"]}", :rifloc => "#{@depositoingr["rifloc"]}", :allevamenti_id => "#{@depositoingr["idallprov"]}", :mod4 => "#{@depositoingr["mod4"]}", :data_mod4 => "#{@depositoingr["datamod4"]}") #:stalla_prov => "#{@depositoingr["stallaprov"]}"
	...
	#		end
			...
			Errore.avviso(finestraingr2, "Bisogna inserire il primo capo della partita prima di procedere con l'inserimento consecutivo.")
		end
	}
...
=end
	...
	bottchiudi.signal_connect("clicked") {
		avviso = Gtk::MessageDialog.new(finestraingr2, Gtk::Dialog::DESTROY_WITH_PARENT, Gtk::MessageDialog::QUESTION, Gtk::MessageDialog::BUTTONS_YES_NO, "Esco dalla procedura di inserimento?")
		risposta = avviso.run
		avviso.destroy
		if risposta == Gtk::Dialog::RESPONSE_YES
			@primocapo = 0
			finestraingr2.destroy
		end
	}
	...
	finestraingr2.show_all
#	labelultima.hide
	if depositoingr["tipomovimento"].to_i == 13 or depositoingr["tipomovimento"].to_i == 32
		#puts "Prima importazione"
		labelspecie.hide
		specie1.hide
		specie2.hide
