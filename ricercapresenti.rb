# Ricerca capi presenti il giorno

def ricercapresenti
	require 'stampapresgiorno'
	mvisperm = Gtk::Window.new("Ricerca capi presenti nel giorno")
...
#			end
		...
		labelcontopres.text = ("Movimenti trovati: #{selpres.length}")
	end

	labelgiorno = Gtk::Label.new("Seleziona il giorno:")
...
#	}
	...
	boxvisp1.pack_start(calendario, false, false, 5)
...
#	regpresenti = Gtk::Button.new( "Visualizza presenti in stalla" )
	cerca = Gtk::Button.new( "Cerca capi" )
#	altrigiornientry = Gtk::Entry.new
	labelcontopres = Gtk::Label.new("Movimenti trovati: 0")
	listapres = Gtk::ListStore.new(Integer, String, String, String, String, String, String, String, String, String, String, String, String)
	vistaperm = Gtk::TreeView.new(listapres)
...
#			#puts giorno
#			#puts giorno.to_date
#			#selpres = Registros.find(:all, :conditions => ["relaz_id= ? and tipouscita IS NULL and dataingresso <= ?", "#{@stallaoper.id}", "#{giorno.strftime("%Y-%m-%d")}"])
#			selpres = Registros.find(:all, :conditions => ["relaz_id= ? and tipouscita IS NULL and dataingresso <= ?", "#{@stallaoper.id}", "#{giorno.to_date}"])
#		else
#			selpres = Registros.find(:all, :select => "*, DATEDIFF(datauscita,dataingresso) as perman", :conditions => ["relaz_id= ? and YEAR(datauscita) = ? and DATEDIFF(datauscita,dataingresso) >= ?", "#{@stallaoper.id}", "#{comboanno.active_iter[0]}", "#{giornipres}"]) #, :order => "perman DESC")
#			#selpres.each do |s|
...
#		end
#		#puts selpres.length
#		ricercapres(selpres, listapres, labelcontopres, valtipo)
#	}
#	reguscite.signal_connect("clicked") {
#		listapres.clear
#		selpres = Registros.find(:all, :conditions => ["relaz_id= ? and tipouscita IS NOT NULL", "#{@stallaoper.id}"], :order => "datauscita, id")
#		#selpres = Registros.find(:all, :conditions => ["relaz_id= ? and tipouscita != ?", "#{@stallaoper.id}", "NULL"], :order => "datauscita, id")
#		ricercapres(selpres, listapres, labelcontopres)
#	}

#	regpresenti.signal_connect("clicked") {
#		listapres.clear
#		selpres = Registros.find(:all, :from => 'registros', :conditions => ["relaz_id= ? and tipouscita IS NULL", "#{@stallaoper.id}"]) # and uscito='0'")
#		ricercapres(selpres, listapres, labelcontopres)
#	}
	cerca.signal_connect("clicked") {
		listapres.clear
		giorno = "#{calendario.year}" + "-" + "#{calendario.month+1}" + "-" + "#{calendario.day}"
		#giornosel = giorno.strftime("%d/%m/%Y")
		#puts giorno.to_date.strftime("%d/%m/%Y")
...
#			giornolibero = Time.parse("#{@giorno.strftime("%Y-%m-%d")}") - altrigiornientry.text.to_i.day
			#puts giornolibero
			#selpres = Registros.find(:all, :conditions => "relaz_id='#{@stallaoper.id}' and marca LIKE '%#{altrigiornientry.text}%'")
			#ActiveRecord::Base.logger = Logger.new(STDOUT)
			#presenti = Registros.find(:all, :conditions => ["relaz_id= ? and tipouscita IS NULL and dataingresso <= ?", "#{@stallaoper.id}", "#{giornolibero.strftime("%Y-%m-%d")}"])
			selpres = Registros.find(:all, :conditions => ["relaz_id= ? and dataingresso <= ? and (datauscita >= ? or datauscita IS NULL)", "#{@stallaoper.id}", "#{giorno}", "#{giorno}"])
			#Registros.find(:all, :conditions => "relaz_id='#{@stallaoper.id}' and marca LIKE '%#{altrigiornientry.text}%'").to_sql
		#else
			#selpres = Registros.find(:all, :conditions => ["relaz_id= ? and YEAR(datauscita) = ? and (datauscita - dataingresso)*86400 >= ?", "#{@stallaoper.id}", "#{comboanno.active_iter[0]}", "#{giornipres}"])
...
#		vistaperm.append_column(colonna14)
	...
	boxvisp2.pack_start(cerca, false, false, 0)
